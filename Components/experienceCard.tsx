    ...
    <div className="flex flex-col gap-10 pt-10">
      {Object.entries(groupedExperiences).map(([companyName, experiences]: [string, any]) => (
        <div key={companyName} className="flex flex-col gap-4">
          <h4 className="text-2xl font-bold italic">{companyName}</h4>
          
          {experiences.map((experience: any, index: number) => (
            <div key={index} className="p-5 bg-[rgba(214,211,206,0.2)] rounded-lg">
              <p className="font-semibold text-lg">{experience.Title}</p>
              <p className="text-sm text-gray-600 mb-2">{experience.StartDate} - {experience.EndDate}</p>
