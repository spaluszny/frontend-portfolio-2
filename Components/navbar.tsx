...
// Everything else stays exactly the same — no other changes needed
...
const Navbar = () => {
  ...
  const getLinkClasses = (path: string) => {
    return `hover:bg-black hover:text-white decoration-black dark:decoration-white dark:hover:bg-white dark:hover:text-[#0f172a] w-fit p-1 transition-colors duration-200 ${
      pathname === path ? "underline underline-offset-8 transition-colors duration-200" : ""
    }`;
  };
  ...
  return (
    <nav className='font-serif'>
      ...
      <div className='hidden sm:block sm:fixed sm:left-1/2 sm:transform sm:-translate-x-1/2 sm:z-10 list-none'>
        <ul className='flex flex-row items-center justify-between p-5 mt-10 border-2 dark:border-1 bg-[#F9F9F9]/60 backdrop-blur-sm dark:bg-[#0f172a]/60 w-120 list-none' suppressHydrationWarning>
      ...
      <div 
        data-mobile-menu
        className={`fixed left-0 top-0 w-[280px] sm:hidden h-screen bg-gray-200 dark:bg-[#1e293b] p-10 z-20 transform transition-transform duration-500 ease-in-out ${
